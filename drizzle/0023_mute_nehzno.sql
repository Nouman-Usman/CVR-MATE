...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='user_brand' AND column_name='ai_dos') THEN
    ALTER TABLE "user_brand" DROP COLUMN "ai_dos";
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='user_brand' AND column_name='ai_donts') THEN
    ALTER TABLE "user_brand" DROP COLUMN "ai_donts";
  END IF;
...
CREATE INDEX "user_video_view_user_idx" ON "user_video_view" USING btree ("user_id");