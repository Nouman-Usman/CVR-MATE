...
export interface CompanyEnrichmentResult {
  ...
  riskFactors: string[];
  idealApproach: Record<string, unknown>;
  keyInsights: string[];
}

/** Full company enrichment profile. Pure generation (no auth/quota/persist/cache). */
export async function generateCompanyEnrichment(params: {
  company: CvrCompany;
  locale: string;
...
${formatBrandContext(brand)}`;
  ...
  try {
    ...
    // Re-throw rate limits — let user know to retry
